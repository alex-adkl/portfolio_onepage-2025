...
const ProjectCard = ({
  ...
  return (
    <>
      {/* Carte Projet */}
      <div
        className="relative cursor-pointer overflow-hidden group w-full h-48 mx-auto rounded-lg sm:h-56 md:h-64 lg:h-45 xl:h-60"
      ...
      >
        ...
        {/* Overlay */}
        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-center items-center text-white rounded-lg">
          <h3 className="text-lg font-bold">{title}</h3>
          <p className="text-xs mt-2">{stacks.join(" • ")}</p>
          <button className="mt-2 bg-white text-black px-3 py-1 rounded-lg text-sm">
            Voir le projet
          </button>
        </div>
      </div>
